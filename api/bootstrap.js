...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const branchResult = await query(
      `select branch_code, branch_name, created_at from branches where branch_code = $1`,
      [branch]
    );
    const settingsResult = await query(
      `select rotation_base_order, rotation_base_monday from branch_settings where branch_code = $1`,
      [branch]
    );
    const branchesResult = await query(
      `select branch_code, branch_name, created_at from branches order by created_at asc`
    );
    ...
    const schedulesResult = await query(
      `select id, schedule_date, hour, hour_text, group_id, group_name, shift_id, shift_name, shift_text, person_id, person_name, slot_order, random_seed
       from schedules
       where branch_code = $1
       order by schedule_date asc, hour asc, group_id asc`,
      [branch]
    );
